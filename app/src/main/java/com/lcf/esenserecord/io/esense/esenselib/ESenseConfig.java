package com.lcf.esenserecord.io.esense.esenselib;
...
public class ESenseConfig {
     ...
     */
    public static GyroLPF parseGyroLPF(byte[] data){
        int lpf_enabled = data[4] & 0x3;
        if(lpf_enabled == 1 || lpf_enabled == 2){
            return(GyroLPF.DISABLED);
        } else {
            GyroLPF lpf = GyroLPF.values()[data[3] & 0x7];
            return(lpf);
        }
    }
     ...
     */
    public static AccLPF parseAccLPF(byte[] data){
        int lpf_enabled = (data[6] & 0x8) >> 3;
        if(lpf_enabled == 1){
            return(AccLPF.DISABLED);
        } else {
            AccLPF lpf = AccLPF.values()[data[6] & 0x7];
            return(lpf);
        }
    }
     ...
     */
    public static GyroRange parseGyroRange(byte[] data){
        return(GyroRange.values()[(data[4] & 0x18) >> 3]);
    }
     ...
     */
    public static AccRange parseAccRange(byte[] data){
        return(AccRange.values()[(data[5] & 0x18) >> 3]);
    }
    ...
    private byte[] setGyroLPFInBytes(byte[] data) {
        if(this.gyroLPF == GyroLPF.DISABLED){
    ...
    private byte[] setAccLPFInBytes(byte[] data) {
        if(this.accLPF == AccLPF.DISABLED){
